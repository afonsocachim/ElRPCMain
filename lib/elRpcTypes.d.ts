export declare enum PayloadResult {
    Success = 0,
    Failure = 1
}
...
export type EventHandler = (event: IpcMainInvokeEvent) => (...args: any[]) => Promise<Payload>;
...
type EventDispatcherMap<T extends DispatcherMap> = {
    [K in keyof T]: (n: (event: IpcRendererEvent, args: Awaited<ReturnType<T[K]>>) => void) => void;
};
type ElRouter = {
